...
pub fn cmd_run() -> Result<(), AppError> {
    ...
    match app.subcommand() {
        ...
        Some(("kctxcreate", sub_cmd)) => exec_kctxcreate(root_dir, sub_cmd)?,
...
fn exec_build(root_dir: &str, argc: &ArgMatches, docker_build: bool) -> Result<(), AppError> {
    ...
    let blocks = argc
        .value_of("blocks")
        .map(|v| v.split(",").into_iter().collect::<Vec<&str>>());
...
fn exec_dpush(root_dir: &str, argc: &ArgMatches) -> Result<(), AppError> {
    ...
    let realm = kdd
        .current_realm()?
        .ok_or_else(|| KddError::DpushFailNoRealm)?;
...
fn exec_kctxcreate(root_dir: &str, argc: &ArgMatches) -> Result<(), AppError> {
    let kdd = load_kdd(root_dir)?;
    let realm = kdd.current_realm()?;
    ...
    if let Some(name) = argc.value_of("name") {
        ...
        println!("Cannot run kubectl config set-context , no ctx name. Use `kdd kctxcreate minikube` for example");
    }
...
fn exec_kexec(root_dir: &str, argc: &ArgMatches) -> Result<(), AppError> {
    ...
    if let Some(realm) = realm {
        ...
        let pod_args = argc.values_of("pod_args").map(|v| {
            v.into_iter()
                .map(|d| d.to_string())
                .collect::<Vec<String>>()
        });
