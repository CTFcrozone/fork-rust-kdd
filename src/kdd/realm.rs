                    ...
                    for path in paths {
                        if let Ok(path) = path {
                            ...
                            if let (Some(stem), Some(ext)) = (
                                ...
                                if path.is_file()
                                    && ext.to_lowercase() == "yaml"
                                    && !stems_set.contains(stem)
                                {
...
impl Realm {
    pub fn from_yaml(kdd_dir: &PathBuf, name: &str, yaml: &Yaml) -> Result<Realm, KddError> {
        let ctx = as_string(yaml, REALM_KEY_CONTEXT)
            .ok_or_else(|| KddError::MissingRealmContext(name.to_string()))?;
